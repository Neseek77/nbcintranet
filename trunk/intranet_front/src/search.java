...
public class search extends HttpServlet {
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String str="'VachanDararith'";  
        try {  
          ResultSet rs=ex.SelectQueryByField(field,table);
          try{
            while(rs.next())
          ...
          out.print("<!DOCTYPE html><html><head>"
                 ...
                 + "<head><script language='javascript' src='js/MultiAction.js'></script>"
                  ...
                  + "$('input#autocomplete').autocomplete({source:["+str+"]});"
                  + "});"
                           + "alert(hello);"
                  + "</script>"
                  + "</head><body style='font-size:62.5%;'><input id='autocomplete'/></body></html>"); 
